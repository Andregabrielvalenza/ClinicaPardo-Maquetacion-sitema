...
@section('content')
	<div class="mainpanel">
		<div class="contentpanel">
			...
			<div class="acordion">
				@can('config-cuenta-empresa')
					<form action="{{route('guardar-config-planes-empresa')}}" method="post" id="form-config" name="form-config">
										...
										@if(count($tipo->planes)>0)
											@foreach($tipo->planes as $plan)
												<div style="width: 100%;display: flex;align-items: center;position: relative;" class="div_acordion">
													<input type="checkbox" name="planes[]" class="mgc mgc-primary mgc-primary-1" data-idch="{{$plan->id}}" value="{{$plan->id}}">
