...
/*
            $('.segurocheck').click(function(){
                var idseguro=$(this).data('ids');
                var idempresa=$(this).data('ide');
                var operacion=$(this).data('ido');
                var elemento=$(this);
                if ($(this).is(':checked'))
                $('#opciones-'+idseguro).css('display','block');
                else {
                    if (operacion==0) {
                        $('#opciones-' + idseguro).css('display', 'none');
                        $('#comision-' + idseguro).val("");
                    }
                    else {
                        if (confirm('Seguro que desea eliminar este producto de su Red')) {
                            $('#loader').addClass('is-active')
                            $.ajax({
                                url: " route('admin.eliminar_comision_empresa') }}?seguro_id=" + idseguro + "&empresa_id=" + idempresa,
                                method: 'GET',
                                success: function (data) {
                                    ...
                                    $('#comision-' + idseguro).val("");
                                    if (data.estado == 1) {
                                        tipo_alerta = 'growl-success';
                                    } else {
                                        tipo_alerta = 'growl-danger';
                                    }
            ...
            $('.grabar-comision').on('keypress', function (e) {
                if(e.which === 13){
                    if ($(this).val()!="") {
                        var elemento_actual=$(this);
                        elemento_actual.attr("disabled", "disabled");
                        var idseguro=$(this).data('ids');
                        var idempresa=$(this).data('ide');
                        var operacion=$(this).data('ido');
                        var url="";
                        if (operacion==0)
                        url= " route('admin.guardar_comision_empresa') }}?seguro_id=" + idseguro+"&empresa_id="+idempresa+"&comision="+elemento_actual.val();
                        else
                        url= " route('admin.actualizar_comision_empresa') }}?seguro_id=" + idseguro+"&empresa_id="+idempresa+"&comision="+elemento_actual.val();
                        ...
                        $.ajax({
                            ...
                            method: 'GET',
                            success: function (data) {
                                elemento_actual.removeAttr("disabled");
                                $('#loader').removeClass('is-active')
                                if (data.estado==1) {
                                    tipo_alerta= 'growl-success';
                                }else {tipo_alerta= 'growl-danger';}
                        ...
                        jQuery.gritter.add({
                            title: "Oppsssssss!",
                            text: "Olvidaste ingresar la comisión",
